...
def start_experiment(config: ExperimentConfig):
    ...
    cur.execute("""
        ...
        (operator_name, sample_name, description, target_duration, target_cycles, max_temp, log_interval, status)
        VALUES (%s, %s, %s, %s, %s, %s, %s, 'WAITING') RETURNING id
    """, (config.operator_name, config.sample_name, config.description, config.duration, config.cycles, config.max_temp, config.interval))
    ...
    pending_command = f"SET:{config.duration}:{config.cycles}:{config.max_temp}:{config.interval}"
...
def insert_data(data: EspSensorData):
    ...
    try:
        ...
        if len(parts) < 6: return {"status": "error_format"}
        ...
        if current_experiment_id: 
            ...
            cur.execute("""
                ...
                (experiment_id, total_time, phase_time, cycle_num, state_code, state_label, ir_temp, tc_temp)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            """, (current_experiment_id, total_time, phase_time, cycle_num, state_code, state_label, ir_temp, tc_temp))
            conn.commit()
            cur.close()
            conn.close()
            return {"status": "saved"}
...
def get_experiment_data(exp_id: int):
    ...
    cur = conn.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        SELECT total_time, phase_time, cycle_num, state_label, ir_temp, tc_temp 
        FROM sensor_logs 
...
def export_csv(exp_id: int):
    ...
    cur.execute("SELECT total_time, phase_time, cycle_num, state_label, ir_temp, tc_temp, recorded_at FROM sensor_logs WHERE experiment_id = %s ORDER BY id ASC", (exp_id,))
    rows = cur.fetchall()
    cur.close()
    conn.close()
    ...
    writer.writerow(["TotalTime", "PhaseTime", "Cycle", "State", "IR_Temp", "TC_Temp", "Recorded At"])
