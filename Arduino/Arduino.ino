/*
 * === PT-KIT — ARDUINO UNO (FULL CONTROLLER) ===
 *
 * This sketch is a thin hardware adapter.  Command parsing, state transitions,
 * actuator writes, calibration persistence, and telemetry live in PTKitController
 * so the host simulator can exercise the same controller without Arduino hardware.
 */
...
void setup() {
  ...
  delay(10); // let TWI bus settle after LCD init
  lcd.setCursor(0,0); lcd.print("Init MLX...");
  mlx.begin();

  delay(10);
  lcd.setCursor(0,0); lcd.print("Init BH1750...");
  lightMeter.begin();
  ...
  while (comm.available()) {
    ...
    if (c == '\n' || cmdIdx >= 79) {
      ...
      if (cmdBuf[0]) {
        ...
        if (strncmp(cmdBuf, "IP:", 3) == 0) {
          lcd.clear();
          lcd.setCursor(0, 0); lcd.print("WiFi Connected!");
          lcd.setCursor(0, 1); lcd.print(&cmdBuf[3]);
          delay(2000);
          lcd.clear();
        } else {
          controller.command(cmdBuf, strlen(cmdBuf));
        }
      }
    } else {
      cmdBuf[cmdIdx++] = c;
